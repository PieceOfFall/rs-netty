...
use rs_netty::{
    codec::{LengthFieldBasedFrameDecoder, LengthFieldPrepender, LineCodec, Utf8DatagramCodec},
    datagram_pipeline, pipeline, Context, DatagramContext, DatagramHandler, Handler, Result,
    TcpServer, UdpServer,
};
use tokio::{
...
async fn main() -> BenchResult<()> {
    ...
    match args.mode.as_str() {
        "server-rs-line" => server_rs_line(args.addr).await?,
        "server-rs-len" => server_rs_len(args.addr).await?,
        "server-rs-udp" => server_rs_udp(args.addr).await?,
...
fn usage() -> String {
    "usage:
  rs-netty-bench server-rs-line --addr localhost:9000
  rs-netty-bench server-rs-len  --addr localhost:9000
  rs-netty-bench server-rs-udp  --addr localhost:9001

  rs-netty-bench client-line --addr localhost:9000 --connections 100 --messages 100000 --payload 128
  rs-netty-bench client-len  --addr localhost:9000 --connections 100 --messages 100000 --payload 128
  rs-netty-bench client-udp  --addr localhost:9001 --connections 100 --messages 100000 --payload 128"
        .to_string()
}

async fn server_rs_line(addr: String) -> BenchResult<()> {
    TcpServer::bind(addr)
        .pipeline(|| pipeline().codec(LineCodec::new()).handler(EchoString))
        .run()
        .await?;
    Ok(())
}

async fn server_rs_len(addr: String) -> BenchResult<()> {
    TcpServer::bind(addr)
        .pipeline(|| {
            pipeline()
                .codec(LengthCodec::new())
                .handler(EchoBytes)
        })
        .run()
        .await?;
...
async fn server_rs_udp(addr: String) -> BenchResult<()> {
    UdpServer::bind(addr)
        .pipeline(|| datagram_pipeline().codec(Utf8DatagramCodec).handler(EchoDatagram))
        .run()
        .await?;
    Ok(())
}
...
impl Handler<Bytes> for EchoBytes {
    type Write = Bytes;

    async fn read(&mut self, ctx: &mut Context<Self::Write>, msg: Bytes) -> Result<()> {
        ctx.write(msg).await
    }
}
...
impl DatagramHandler<String> for EchoDatagram {
    type Write = String;
...
impl LengthCodec {
    fn new() -> Self {
        Self {
            decoder: LengthFieldBasedFrameDecoder::new(8 * 1024 * 1024, 0, 4),
            encoder: LengthFieldPrepender::new(4),
        }
    }
}

impl rs_netty::codec::Decoder for LengthCodec {
    type Item = Bytes;
...
impl rs_netty::codec::Encoder<Bytes> for LengthCodec {
